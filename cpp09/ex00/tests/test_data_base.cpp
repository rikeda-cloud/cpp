...
static bool _test_simple_db(void) {
  ...
  if (db.select("2000-01-01") != 1 || db.select("2000-01-10") != 10 ||
    ...
    std::cout << "[KO] _test_simple_db: equal case" << std::endl;
    return true;
  }
  ...
  if (db.select("1999-12-31") != DataBase::NOT_FOUND ||
    ...
    std::cout << "[KO] _test_simple_db: not equal case" << std::endl;
    return true;
  }
...
static bool _test_empty_db(void) {
  ...
  if (db.select("2000-01-01") != DataBase::NOT_FOUND) {
    std::cout << "[KO] _test_empty_db: no empty" << std::endl;
    return true;
  }
  std::cout << "[OK]" << std::endl;
  return false;
...
static bool _test_insert_multiple_key(void) {
  ...
  if (db.select("2000-01-01") != 100) {
    std::cout << "[KO] _test_insert_multiple_key: not overwritten" << std::endl;
    return true;
  }
  std::cout << "[OK]" << std::endl;
  return false;
